...
namespace Easy.Caching {
    public class DefaultAsyncTokenProvider : IAsyncTokenProvider {
        public DefaultAsyncTokenProvider(ILoggerFacade logger) {
            Logger = logger;
        }
        ...
        public class AsyncVolativeToken : IVolatileToken {
            ...
            public AsyncVolativeToken(Action<Action<IVolatileToken>> task, ILoggerFacade logger) {
                _task = task;
                Logger = logger;
            }
            ...
            public void QueueWorkItem() {
                ...
                ThreadPool.QueueUserWorkItem(state => {
                    try {
                        ...
                        Logger.Log(ex.Message+ "Error while monitoring extension files. Assuming extensions are not current.",Category.Exception,Priority.High);
                        _taskException = ex;
                    }
                    finally {
